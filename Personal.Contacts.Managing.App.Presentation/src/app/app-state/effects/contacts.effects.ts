        ...
        mergeMap(([action, latest]) => {
            return this.contactService.getAllContacts()
                .pipe(
                    ...
                    catchError((errorResponse) => of(contactsActions.setErrorMessage({ errorMessage: errorResponse.error })))
                )
        })
    )
    );
    ...
    createContact$ = createEffect(() => this.actions$.pipe(
        ...
        mergeMap(([action, latest]) => {
            return this.contactService.createContact(latest.contact!)
                .pipe(
                    map((contact) => (contactsActions.createContactSuccess({ contact: contact, result: true }))),
                    catchError((errorResponse) => of(contactsActions.setErrorMessage({ errorMessage: errorResponse.error })))
                )
        })
    )
    ...
    updateContact$ = createEffect(() => this.actions$.pipe(
        ...
        mergeMap(([action, latest]) => {
            return this.contactService.updateContact(latest.contact!)
                .pipe(
                    map((contact) => (contactsActions.updateContactSuccess({ contact: contact, result: true }))),
                    catchError((errorResponse) => of(contactsActions.setErrorMessage({ errorMessage: errorResponse.error })))
                )
        })
    )
